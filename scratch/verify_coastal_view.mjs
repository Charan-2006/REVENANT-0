...
import { INITIAL_VESSELS } from '../src/data/vessels.ts';
...
for (const ts of testStates) {
  ...
  assert(
    res.displayStatus === ts.expectedDisplay,
    `State combination (${ts.correlation} + ${ts.inside ? 'INSIDE' : 'OUTSIDE'}) -> displayStatus: ${ts.expectedDisplay}`,
    res.displayStatus
  );
